...
def createBuildingsGeojson(geojson, id):
    ...
    polys_latlng = convert_polygons_to_latlon(
        polygons, thresh_res.size, mosaic_bb)
    # Return Polygons
    building_geojsons = convertToGeoJsons(polys_latlng)
    outputjson_string = '{{"type" : "GeometryCollection", "geometries" : [{}]}}'.format(
        ','.join(building_geojsons))
