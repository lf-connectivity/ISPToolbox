    ...
    def get(self, request, task_id):
        ...
        if pipeline:
            filtered_fields = list(
                filter(
                    lambda x: (
                        x != 'output_geometryCollection' and x != 'input_geometryCollection'), [
                        field.name for field in pipeline._meta.fields]))
            response_data['progress'] = model_to_dict(
                pipeline, fields=filtered_fields)
...
class ResultView(View):
    def get(self, request, task_id):
        pipeline = BuildingDetection.objects.get(task=task_id)
        if pipeline and pipeline.output_geometryCollection is not None:
