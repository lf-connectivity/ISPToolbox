    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        with open(en_path, encoding='ISO-8859-15') as fread:
            ...
            for lst in lines:
                ...
                # need owner, not representative. there are multiple entries per
                # reg number for that.
                entity_type = lst[5].strip()

                if registration_number and owner_name and entity_type == 'O':
    ...
    if settings.PROD:
        ...
        try:
            with tempfile.TemporaryFile() as f:
                f.write(contents)
                f.seek(0)
                resp, data = uploadNewTileset(f, ASR_MAPBOX_OVERLAY)
