 ...
 */
export function createFresnelZone(
    num_samples: number,
    resolution: number = 1.0,
    freq_ghz: number = 60.0,
    fresnel_zone_number: number = 1.0
) {
    ...
    for (let i = 0; i < num_samples; i++) {
        ...
        const d2 = (num_samples - i) * resolution;
        const d_total = num_samples * resolution;
...
export function calculateMaximumFresnelRadius(
    distance_m: number,
    frequency_ghz: number,
    fresnel_number: number = 1.0
): number {
 ...
 */
export function adjustProfileCurvatureEarth(
    tx_hgt_m: number,
    rx_hgt_m: number,
    num_samples: number,
    resolution: number = 1.0
    ...
    const theta1 = 0;
    const theta2 = (num_samples * resolution) / earth_radius_m;
...
export function createLinkProfile(
    ...
    frequency_ghz: number = 0.0
): { los: Array<number>; fresnel: Array<[number, number, number]> } {
    ...
    return {
        los: profile,
        fresnel: fresnel_zone.map((v, idx) => {
            return [idx, profile[idx] - v, profile[idx] + v];
        })
    };
}

export function findLidarObstructions(
    fresnel: Array<[number, number, number]>,
    lidar: Array<number>,
    resolution: number = 1.0
): Array<[number, number]> {
    const fresnel_bottom = fresnel.map((x) => {
        return x[1];
    });
    const overlaps: Array<[number, number]> = [];
    const fresnel_distances = fresnel.map((x) => {
        return x[0];
    });
    const lidar_distances = lidar.map((x, idx) => {
        return resolution * idx;
    });
    const lidar_values = lidar;
    const fresnel_val_interp = everpolate.linear(
        lidar_distances,
        fresnel_distances,
        fresnel_bottom
    );
...
export function findOverlaps(
    link: Array<number>,
    lidar: Array<[number, number]>,
    resolution: number = 1.0
): Array<[number, number]> {
    const overlaps: Array<[number, number]> = [];
    const link_distances = link.map((_, idx) => {
        return idx * resolution;
    });
    const lidar_distances = lidar.map((x) => {
        return x[0];
    });
    const lidar_values = lidar.map((x) => {
        return x[1];
    });
