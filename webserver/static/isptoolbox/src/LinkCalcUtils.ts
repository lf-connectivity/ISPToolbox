 ...
 */
export function createFresnelZone(num_samples : number, resolution : number = 1.0, freq_ghz : number = 60.0, fresnel_zone_number : number = 1.0){
 ...
 */
export function adjustProfileCurvatureEarth(tx_hgt_m : number, rx_hgt_m : number, num_samples: number, resolution : number = 1.0): Array<number>
{
    ...
    frequency_ghz : number = 0.0): {los: Array<number>, fresnel: Array<[number, number, number]>}
{
    ...
    return {los: profile, fresnel: fresnel_zone.map((v, idx) => {return [idx, profile[idx] - v, profile[idx] + v]})};

}

export function findLidarObstructions(fresnel : Array<[number, number, number]>, lidar: Array<number>, resolution: number = 1.0): Array<[number, number]> {
    const fresnel_bottom = fresnel.map((x)=>{return x[1];});
    const overlaps: Array<[number, number]> = [];
    const fresnel_distances = fresnel.map((x)=> {return x[0];});
    const lidar_distances = lidar.map((x, idx)=>{return resolution * idx;});
