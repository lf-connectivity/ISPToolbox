...
def calculateSectorNearby(sector_id: str):
    sector = workspace_models.AccessPointSector.objects.get(uuid=sector_id)
    building_coverage, created = workspace_models.AccessPointCoverageBuildings.objects.get_or_create(
        sector=sector)
    if created:
...
def calculateSectorCoverage(sector_id: str):
    ...
    with tempfile.NamedTemporaryFile(suffix=".tif") as fp:
        ...
        with rasterio.open(fp.name) as ds:
            ...
            coverage = workspace_models.AccessPointCoverageBuildings.objects.filter(
                sector=sector
            ...
            for idx, building in enumerate(coverage.nearby_buildings.all()):
                ...
                building_polygon = (
                    ...
                    gis_data_models.MsftBuildingOutlines.objects.get(id=building.msftid).geog
                )
                ...
                if building_polygon:
                    ...
                    if viewshed.mode == workspace_models.Viewshed.CoverageStatus.GROUND:
                        serviceable = (np.logical_and(out_image < sector.default_cpe_height, out_image != np.Inf)).any()
                    if viewshed.mode == workspace_models.Viewshed.CoverageStatus.NORMAL:
                        serviceable = (out_image == VISIBLE_PIXEL_VALUE).any()
                    if serviceable:
                        building.status = workspace_models.BuildingCoverage.CoverageStatus.SERVICEABLE
                    else:
                        building.status = workspace_models.BuildingCoverage.CoverageStatus.UNSERVICEABLE
                    if serviceable:
