...
class SessionCreateUpdateView(
    mixins.CreateModelMixin, mixins.UpdateModelMixin, generics.RetrieveAPIView
):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.query_params.get("format") == "json":
            return response
        return redirect(
            "workspace:edit_network", response.data["uuid"], response.data["name"]
        )
...
class SessionListView(mixins.ListModelMixin, generics.RetrieveAPIView):
    serializer_class = WorkspaceMapSessionSerializer
...
class SessionDeleteView(mixins.DestroyModelMixin, generics.RetrieveAPIView):
    serializer_class = WorkspaceMapSessionSerializer
    lookup_field = "uuid"
    permission_classes = [AllowAny]
    schema = None
...
class SessionDownloadGeoJSONView(RegularUserRequiredMixin, View):
    def get(self, request, session_uuid=None):
        ...
        response[
            "Content-Disposition"
        ] = f'attachment; filename="{session.name}.geojson"'
        return response


class SessionDownloadKMZView(RegularUserRequiredMixin, View):
    def get(self, request, session_uuid=None):
        session = get_object_or_404(
            WorkspaceMapSession, owner=request.user, uuid=session_uuid
        )
...
class SessionSaveAsView(RegularUserRequiredMixin, View):
        ...
        if saveas_form.is_valid():
            ...
            try:
                ...
                return JsonResponse(
                    {
                        "url": reverse(
                            "workspace:edit_network", args=[session.uuid, session.name]
                        )
                    }
                )
            except IntegrityError:
                return JsonResponse(
                    {"error": WorkspaceMapSession.UNIQUE_TOGETHER_ERROR}, status=400
                )
